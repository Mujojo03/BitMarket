        ...
        <Card key={order.id} className="bg-gray-800 border-gray-700">
          <CardHeader>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <CardTitle className="text-lg">Order #{order.id.toString().slice(-8)}</CardTitle>
                <CardDescription>
              ...
              <div key={product.id} className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <Link href={`/product/${product.id}`} className="font-medium hover:text-bitcoin">
                      {product.name}
                    </Link>
                    <div className="text-sm text-gray-400">{formatSats(product.priceSats)} sats</div>
                  </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="text-lg font-bold text-bitcoin">
                  {formatSats(
                    order.products.reduce((sum, product) => sum + product.priceSats, 0)
                  )} sats
                </p>
              </div>
              {order.status === "paid" && (
