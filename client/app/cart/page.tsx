...
import { buyNow, checkoutComplete, calculateTotalPrice, updateItemQuantity, removeItemFromCart } from "@/contexts/cartService"
...
export default function CartPage() {
  ...
  try {
    ...
    // Show payment_request to user or QR code
  } catch (error) {
...
// Function to handle checkout
  ...
  try {
    ...
    if (typeof result === "object" && result !== null && "message" in result) {
      // ts-expect-error: TypeScript can't infer the type of result.message
      console.log(result.message);
    } else {
      console.log("Checkout complete, but no message returned.");
    }
  } catch (error) {
    if (error instanceof Error) {
...
const updateItemQuantity = async (cartItemId: string, quantity: number) => {
  try {
    const res = await fetch(`/api/cart/update/${cartItemId}`, {
      ...
      body: JSON.stringify({ 
        userId: user?.id || "user1", // For demo purposes
        cartItemId,
        quantity,
      }),
    })
...
const calculateTotalPrice = async () => {
  try {
    ...
    if (!res.ok) throw new Error('Failed to calculate total.')
    ...
    toast({
      variant: 'destructive',
      title: 'Error',
      description: 'Failed to calculate total. Please try again.',
    })
  }
}
...
// Function to remove item from cart
  ...
  try {
    const res = await fetch(`/api/cart/remove/${cartItemId}`, {
      ...
      body: JSON.stringify({ userId: user?.id || "user1" }), // For demo purposes
    })
    if (!res.ok) {
            ...
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <div className="flex items-center">
                    <Bitcoin className="h-4 w-4 text-bitcoin mr-1" />
                    <span>{calculateTotalPrice().toLocaleString()} sats</span>
                  </div>
                </div>
                <div className="flex justify-between">
  ...
  function setTotalPrice(totalPrice: any) {
