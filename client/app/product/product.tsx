...
import type { Product, Category, Seller, User } from '@/lib/types'

export default function ProductPage() {
  const { id } = useParams()
  const { getProductById } = useProductContext()

  // Product with Category and Seller (Seller not User)
  const [product, setProduct] = useState<(Product & { category: Category; seller: Seller }) | null>(null)
  const [relatedProducts, setRelatedProducts] = useState<(Product & { category: Category; seller: Seller })[]>([])
  const [reviews, setReviews] = useState<{ user: User; rating: number; comment: string }[]>([]) // mock structure
    ...
    if (id) {
      getProductById(id as string).then((data) => {
        if (data) {
          ...
          // setReviews(data.reviews) // Uncomment when Review API ready
        }
      })
    }
  }, [id, getProductById])
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">by {product.seller.username}</p>
        <Image src={product.imgUrl} alt={product.name} width={600} height={400} className="rounded" />
        <p className="mt-4">{product.description}</p>
        <p className="text-xl font-semibold mt-2">{product.priceSats.toLocaleString()} sats</p>
      ...
      <div className="mt-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {relatedProducts.map((relatedProduct) => (
            <div key={relatedProduct.id} className="border p-4 rounded shadow-sm">
              <Image
              ...
              <p className="text-gray-600">{relatedProduct.priceSats.toLocaleString()} sats</p>
              <p className="text-sm text-gray-500">Category: {relatedProduct.category.name}</p>
              <p className="text-sm text-gray-500">Seller: {relatedProduct.seller.username}</p>
            </div>
          ))}
        </div>
      </div>
